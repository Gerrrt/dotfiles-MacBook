...
3. **Green the gate.** `make lint` bundles the repo-owned checks locally and
   ...
   | `make exec-bits` | a shebang and the exec bit agree, both ways, read from the git index |
   | file hygiene (CI) | `.pre-commit-config.yaml`'s upstream hooks over the whole tree — whitespace, EOF newlines, line endings, merge markers, yaml/json/toml |
