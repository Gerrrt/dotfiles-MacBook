...
directory is ignored rather than committed by accident. `gitleaks` scans the repo-owned
tree on every PR in CI, and locally at commit time **once you have run `pre-commit
install`** — the local hook is opt-in, CI is not.
